...
# CNN implementation
def cnn(image, conv_filter, pool_size, pool_stride):
    output = convolution(image, conv_filter, 1)
...
for epoch in range(epochs):
    for i, (image, label) in enumerate(zip(train_images, train_labels)):
        cnn_output = cnn(image, conv_filters[0], pool_size, pool_stride)
...
test_cnn_outputs = np.array([cnn(image, conv_filters[0], pool_size, pool_stride) for image in test_images])
