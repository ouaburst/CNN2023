...
def max_pooling_backward(d_output_tensor, pooled_map, pool_size, stride):
    h, w = pooled_map.shape
    d_image = np.zeros_like(pooled_map)
    d_output = d_output_tensor[0]  # Extract the 2D array from the tensor

    for y in range(0, d_output.shape[0] * stride, stride):
        for x in range(0, d_output.shape[1] * stride, stride):
            img_region = pooled_map[y : y + pool_size, x : x + pool_size]
            if img_region.size > 0: # Add this check to ensure img_region is not empty
                mask = (img_region == np.max(img_region))
                d_image[y : y + pool_size, x : x + pool_size] += (d_output[y // stride, x // stride] * mask).reshape(pool_size, pool_size)
...
for epoch in range(epochs):
    ...
    for i, (image, label) in enumerate(zip(train_images, train_labels)):
        ...
        # Calculate the gradients for the fully connected layer
        d_fc_output = np.dot(d_input, fc_weights.T)
        fc_weights -= learning_rate * np.dot(cnn_output.reshape(1, -1).T, d_input)
        fc_biases -= learning_rate * np.sum(d_input, axis=0, keepdims=True)
