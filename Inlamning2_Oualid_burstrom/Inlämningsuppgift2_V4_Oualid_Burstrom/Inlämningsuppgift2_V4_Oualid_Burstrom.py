...
for epoch in range(epochs):
    ...
    for i, (image, label) in enumerate(zip(train_images, train_labels)):
        ...
        feature_maps = [convolution(image, conv_filter, 2) for conv_filter in conv_filters]
        pooled_maps = [max_pooling(feature_map, 3, 2) for feature_map in feature_maps]
        cnn_output = np.concatenate([pooled_map.flatten() for pooled_map in pooled_maps])
        ...
        # Update the convolutional filters
        for j, conv_filter in enumerate(conv_filters):
            for y in range(0, feature_maps[j].shape[0]):
                for x in range(0, feature_maps[j].shape[1]):
                    img_region = image[y * 2 : y * 2 + 5, x * 2 : x * 2 + 5]
                    conv_filters[j] -= learning_rate * d_input[0, j] * img_region
