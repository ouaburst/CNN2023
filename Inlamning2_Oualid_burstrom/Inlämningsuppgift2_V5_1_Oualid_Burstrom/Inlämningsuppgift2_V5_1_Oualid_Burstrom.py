...
class ConvLayer(Layer):
    ...
    def forward_propagation(self, input):
        ...
        for k in range(self.layer_depth):
            for d in range(self.input_depth):
                kernel = self.weights[:,:,d,k]
                self.output[:,:,k] += convolution2d(self.input[:,:,d], kernel) + self.bias[k]
    ...
    def backward_propagation(self, output_error, learning_rate):
        ...
        for k in range(self.layer_depth):
            for d in range(self.input_depth):
                in_error[:,:,d] += convolution2d(output_error[:,:,k], np.flip(self.weights[:,:,d,k]), 'full')
                dWeights[:,:,d,k] = convolution2d(padded_input[:,:,d], output_error[:,:,k], 'valid')                
